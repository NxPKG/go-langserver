...
import (
	...
	"github.com/nxpkg/ctxvfs"
	"github.com/nxpkg/go-langserver/gituri"
	"github.com/nxpkg/go-langserver/langserver"
	"github.com/nxpkg/go-langserver/pkg/lsp"
	"github.com/nxpkg/go-lsp/lspext"
	"github.com/nxpkg/jsonx"
)
...
func detectCustomGOPATH(ctx context.Context, fs ctxvfs.FileSystem) (gopaths []string) {
	...
	if paths := detectNxpkgGOPATH(ctx, fs); len(paths) > 0 {
		return paths
	}
...
// detectNxpkgGOPATH tries to detect monorepos which require their own custom
// GOPATH. We detect a .nxpkg/config.json file with the following
// contents:
//
//	{
//	  "go": {
...
//	}
//
// See the nxpkgConfig struct documentation for more info.
//
// This is best-effort. If any errors occur or we do not detect a custom
// gopath, an empty result is returned.
func detectNxpkgGOPATH(ctx context.Context, fs ctxvfs.FileSystem) (gopaths []string) {
	cfg := readNxpkgConfig(ctx, fs)
...
// nxpkgConfig is a struct representing the Go portion of the
// .nxpkg/config.json file that may be placed in the repo root.
type nxpkgConfig struct {
	...
	Go struct {
		...
		//
		// See https://github.com/nxpkg/go-langserver#custom-gopaths-go-monorepos.
		GOPATH []string
		...
		//
		// See https://github.com/nxpkg/go-langserver#vanity-import-paths.
		RootImportPath string
	} `json:"go"`
}

// readNxpkgConfig reads the .nxpkg/config.json file from the
// repository root if it exists, otherwise an empty struct value (not nil).
func readNxpkgConfig(ctx context.Context, fs ctxvfs.FileSystem) *nxpkgConfig {
	config := nxpkgConfig{}
	b, err := ctxvfs.ReadFile(ctx, fs, "/.nxpkg/config.json")
	if err != nil {
		return &config
	}
...
func determineRootImportPath(ctx context.Context, originalRootURI lsp.DocumentURI, fs ctxvfs.FileSystem) (rootImportPath string, err error) {
	...
	// If .nxpkg/config.json specifies a root import path to use, then
