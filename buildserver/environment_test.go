	...
	cases := []tcase{
		...
		},
		{
			...
			FS: map[string]string{
				...
				".nxpkg/config.json": `{"go": {"RootImportPath": "good.org/code"}}`,
			},
		},
		{
			...
			FS: map[string]string{
				...
				".nxpkg/config.json": `{"go": {"RootImportPath": "alice.org/code"}}`,
			},
		},
		{
			Name:           "fallback",
...
`,
		...
		{
			Name:           "monorepo_nxpkg_config",
			RootURI:        "git://github.com/kim/monorepo",
			WantImportPath: "",
			WantGoPath:     "/workspace/third_party:/workspace/code:/",
			FS: map[string]string{
				".nxpkg/config.json": `{"go": {"GOPATH": ["/third_party", "code"]}}`,
			},
		},
	}
