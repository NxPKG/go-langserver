...
import (
	...
	gobuildserver "github.com/nxpkg/go-langserver/buildserver"
...
//   go test github.com/nxpkg/nxpkg/xlang -bench=Integration -benchmem -run='^$' > /tmp/BenchmarkIntegration.old.txt
//
//   # Run this after you've made the changes that you want to benchmark:
//   go test github.com/nxpkg/nxpkg/xlang -bench=Integration -benchmem -run='^$' | tee /tmp/BenchmarkIntegration.new.txt && benchcmp /tmp/BenchmarkIntegration.{old,new}.txt
