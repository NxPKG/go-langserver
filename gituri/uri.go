...
// manipulate Nxpkg-specific components. All URIs are valid
// URLs, but Nxpkg assigns special meaning to certain URL components as described below.
//
// Nxpkg URIs can refer to repos (at an optional revision), or a
...
//
// A Nxpkg URI is not guaranteed (or intended) to be a unique or
...
func Parse(uriStr string) (*URI, error) {
	...
	if !u.IsAbs() {
		return nil, &url.Error{Op: "gituri.Parse", URL: uriStr, Err: errors.New("nxpkg URI must be absolute")}
	}
	return &URI{*u}, nil
}
